    ...
    <div className="space-y-5 pb-10">
      ...
      {step === 3 && context && <section className="rounded-xl border bg-card p-5 shadow-sm">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {[{ label: "Monthly Dues", value: monthlyDues, set: setMonthlyDues, disabled: !canOverride }, ...(hasActiveCashPabaon ? [{ label: "Cash Pabaon", value: cashPabaon, set: setCashPabaon, disabled: !canOverride }] : []), { label: "Cash Assistance Loan Deduction", value: loanDeduction, set: setLoanDeduction, disabled: !context.activeLoan }].map((item) => <div key={item.label} className="rounded-xl border p-4"><Label>{item.label}</Label><div className="relative mt-3"><span className="absolute left-3 top-2 text-sm text-muted-foreground">₱</span><Input className="pl-7" type="number" min="0" step="0.01" value={item.value} disabled={item.disabled} onChange={(e) => item.set(Number(e.target.value))} /></div>{item.disabled && item.label !== "Cash Assistance Loan Deduction" && <p className="mt-2 text-xs text-muted-foreground">Requires payroll.manual.override</p>}</div>)}
        </div>
