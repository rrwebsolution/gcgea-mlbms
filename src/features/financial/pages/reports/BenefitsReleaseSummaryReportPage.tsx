...
export default function BenefitsReleaseSummaryReportPage() {
  const [draft, setDraft] = React.useState<Filters>(EMPTY_FILTERS)
  const [applied, setApplied] = React.useState<Filters | null>(null)

  const rows = React.useMemo<BenefitReleaseRow[]>(() => {
    if (!applied) return []
    return getAllBenefits()
      .filter((b) => !!b.releaseDate)
      .filter((b) => {
        if (applied.dateFrom && b.releaseDate! < applied.dateFrom) return false
        if (applied.dateTo && b.releaseDate! > applied.dateTo) return false
      ...
      .map((b) => ({
        ...
        releasedAmount: b.approvedAmount ?? b.requestedAmount,
      }))
  ...
  function handleExportCsv() {
    downloadCsv(
      "benefits-release-summary-report.csv",
    ...
    toast.success("Benefits release summary exported to CSV.")
  }

  const columns: ColumnDef<BenefitReleaseRow, unknown>[] = [
    { accessorKey: "applicationNumber", header: "Application #", cell: ({ row }) => <Link to={`/benefits/${row.original.id}`} className="font-medium text-foreground hover:text-primary hover:underline">{row.original.applicationNumber}</Link> },
  ...
  return (
    <div className="space-y-5">
      ...
      <PageHeader title="Benefits Release Summary" description="Review benefit claims that have been released, by amount, office, and date." />
      ...
      {!applied ? (
        <div className="rounded-xl border border-dashed border-border bg-muted/30 p-10 text-center">
          <p className="text-sm text-muted-foreground">Set your filters and click <strong className="text-foreground">Generate</strong> to build the benefits release summary.</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <StatCard label="Total Released" value={formatCurrency(summary.totalReleased)} icon={Banknote} tone="success" />
            <StatCard label="Benefits Released" value={String(summary.count)} icon={Hash} tone="primary" />
          ...
          <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
            ...
            {monthlyData.length === 0 ? (
              <p className="py-10 text-center text-sm text-muted-foreground">No released benefits in range.</p>
            ) : (
          ...
          <div className="rounded-xl border border-border bg-card shadow-sm">
            <DataTable
              columns={columns}
              data={rows}
              emptyTitle="No released benefits match your filters"
