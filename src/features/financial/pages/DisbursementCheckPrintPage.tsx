...
import { ArrowLeft, Info, Printer, WalletCards } from "lucide-react"
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-border/60 bg-card/90 p-4 shadow-xs backdrop-blur-xs print:hidden">
        <div className="flex items-center gap-3">
          <Button
            ...
            render={<Link to={`/financial/disbursements/${disbursement.id}`} />}
          >
            <ArrowLeft className="size-3.5" /> Back to Disbursement
          </Button>
          ...
          <div className="flex items-center gap-2">
            ...
            <StatusBadge label={disbursement.status} tone={tone(disbursement.status)} />
          </div>
        </div>
