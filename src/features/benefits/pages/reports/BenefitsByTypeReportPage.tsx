...
import { ArrowLeft, Banknote, Download, RotateCcw, Tag } from "lucide-react"
...
export default function BenefitsByTypeReportPage() {
  const [draft, setDraft] = React.useState<Filters>(EMPTY_FILTERS)
  const [applied, setApplied] = React.useState<Filters | null>(null)

  const rows = React.useMemo<TypeRow[]>(() => {
    if (!applied) return []
    ...
    for (const b of filtered) {
      const entry = map.get(b.benefitTypeName) ?? { count: 0, totalRequested: 0, totalApproved: 0 }
      entry.count += 1
      entry.totalRequested += b.requestedAmount
      entry.totalApproved += b.approvedAmount ?? 0
      map.set(b.benefitTypeName, entry)
    }
  ...
  function handleExportCsv() {
    downloadCsv(
      "benefits-by-type-report.csv",
      ["Benefit Type", "Applications", "Total Requested", "Total Approved", "Share %"],
      rows.map((r) => [r.benefitType, r.count, r.totalRequested.toFixed(2), r.totalApproved.toFixed(2), r.share.toFixed(1)])
    )
    toast.success("Benefits by type report exported to CSV.")
  }
      ...
      {!applied ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <StatCard label="Types Represented" value={String(summary.types)} icon={Tag} tone="primary" />
            <StatCard label="Total Requested" value={formatCurrency(summary.totalRequested)} icon={Banknote} tone="success" />
            <StatCard label="Top Benefit Type" value={summary.topType} icon={Tag} tone="gold" />
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              ...
              {rows.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={320}>
                  <BarChart data={rows.slice(0, 6)} layout="vertical" margin={{ top: 4, right: 24, left: 8, bottom: 4 }}>
                    ...
                    <YAxis type="category" dataKey="benefitType" width={120} tick={{ fill: "var(--color-foreground)", fontSize: 11 }} tickLine={false} axisLine={false} />
            ...
            <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
              ...
              {rows.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={320}>
                  <PieChart>
                    <Pie data={rows.slice(0, 6)} dataKey="totalRequested" nameKey="benefitType" cx="50%" cy="50%" outerRadius={90} label={(props: { name?: string; percent?: number }) => `${props.name} (${((props.percent ?? 0) * 100).toFixed(0)}%)`}>
                      {rows.slice(0, 6).map((entry, idx) => <Cell key={entry.benefitType} fill={PIE_COLORS[idx % PIE_COLORS.length]} />)}
                    </Pie>
